...
var builtinRules = []Rule{
	...
	},
	{
		...
		Severity:        "CRITICAL",
		Regex:           MustCompile(`(?i)AccountEndpoint\s*=\s*https://[^;]+\.(?:documents|cosmos)\.azure\.com[^;]*\s*;\s*AccountKey\s*=\s*(?P<secret>[A-Za-z0-9+/]{86}==)`),
		SecretGroupName: "secret",
	...
	},
	{
		...
		Severity:        "CRITICAL",
		Regex:           MustCompile(`(?i)Endpoint\s*=\s*https://[a-zA-Z0-9\-]+\.azconfig\.io\s*;\s*Id\s*=\s*[a-zA-Z0-9+/=:\-]+\s*;\s*Secret\s*=\s*(?P<secret>[a-zA-Z0-9+/=~]+)`),
		SecretGroupName: "secret",
